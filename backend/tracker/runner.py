...
def _run_tracking_process_task(output_queue: Queue, queue_to_tracker: Queue, stop_event: MpEvent, normalized_id: str):
    title = f"DynamiX-Labs - SatelliteTracker[{normalized_id}]"
...
class TrackerSupervisor:
    ...
    def _build_process(self, tracker_id: str, queue_to_tracker: Queue, stop_event: MpEvent):
        ...
        return multiprocessing.Process(
            target=_run_tracking_process_task,
            args=(self.output_queue, queue_to_tracker, stop_event, normalized_id),
            name="DynamiX-Labs - SatelliteTracker",
        )
