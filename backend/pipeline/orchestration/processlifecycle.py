...
class NamedWorkerWrapper:
    def __init__(self, worker_func, process_name):
...
def _create_named_worker_process(worker_func, process_name, *args):
    ...
    """
    return NamedWorkerWrapper(worker_func, process_name)
...
class ProcessLifecycleManager:
    ...
    async def start_sdr_process(self, sdr_device, sdr_config, client_id):
        ...
        if sdr_device["type"] == "rtlsdrusbv3":
            ...
            process_name = f"DynamiX-Labs - RTL-SDR-USB-v3-{sdr_id}"
            ...
            driver = None
            worker_process = rtlsdr_worker_process
            process_name = f"DynamiX-Labs - RTL-SDR-TCP-v3-{sdr_id}"

        elif sdr_device["type"] == "rtlsdrusbv4":
            connection_type = "usb"
            driver = None
            worker_process = rtlsdr_worker_process
            process_name = f"DynamiX-Labs - RTL-SDR-USB-v4-{sdr_id}"
            ...
            driver = None
            worker_process = rtlsdr_worker_process
            process_name = f"DynamiX-Labs - RTL-SDR-TCP-v4-{sdr_id}"
            ...
            process_name = f"DynamiX-Labs - SigMF-Playback-{sdr_id}"
        ...
        if sdr_id in self.processes and self.processes[sdr_id]["process"].is_alive():
            ...
            fft_process_name = f"DynamiX-Labs - FFT-Processor-{sdr_id}"
